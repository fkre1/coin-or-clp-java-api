...
CLPModel_setObjective(CLPModel *self, const double objective[], int len) {
  self->setObjective(objective, len);
}

extern "C" DLL_EXPORT void CLPModel_setTimeLimit(CLPModel *self,
                                                 double seconds) {
...
extern "C" DLL_EXPORT void CLPModel_addFullRow(CLPModel *self,
                                               const double row[], int len,
                                               double lb, double ub) {
  self->addFullRow(row, len, lb, ub);
}
...
extern "C" DLL_EXPORT void
CLPModel_addSparseRows(CLPModel *self, int numrows, const int rowstarts[],
                       const double elems[], const int indices[], int len,
                       const double lb[], const double ub[]) {
  self->addSparseRows(numrows, rowstarts, elems, indices, len, lb, ub);
}
extern "C" DLL_EXPORT int CLPModel_solve(CLPModel *self) {
  return static_cast<int>(self->solve());
}
extern "C" DLL_EXPORT const double *
CLPModel_getColSolution(const CLPModel *self) {
